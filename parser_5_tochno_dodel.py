...
def on_combo_select(event):
    ...
    for i, link in enumerate(obj_pool, 1):
        ...
        # Применяем тег к заголовку
        end_index = output_box.index(END)
        ...
        # Создаем замыкание с текущим объектом
...
def show_article_details(link_obj):
...
combo = ttk.Combobox(root, state="readonly", width=100)
combo.set("Выберите")
combo.place(x=150, y=16)
combo.bind("<<ComboboxSelected>>", on_combo_select)

output_box = Text(root, height=19, width=109)
output_box.place(x=10, y=95)
output_box.config(state=DISABLED)

output_box2 = Text(root, height=10, width=109)
